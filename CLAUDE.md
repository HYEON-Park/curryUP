...
- 세션 시작 시 `PROGRESS.md`를 먼저 읽고 현재 작업 상태를 파악한다.
- 작업 종료 시 또는 주요 단계 완료 시 `PROGRESS.md`를 갱신한다.
  - "현재 작업 중" / "다음 할 일" / "최근 완료" 섹션을 실제 상태에 맞게 이동한다.
  - 갱신 시 상단의 "마지막 갱신" 날짜도 바꾼다.
- 설계상 되돌리기 어려운 결정은 "주요 결정"에 배경과 함께 한 줄로 남긴다.
