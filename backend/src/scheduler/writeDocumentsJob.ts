...
function runClaudeWriteDocuments(userId: string, todayKey: string): Promise<void> {
  ...
  const prompt = [
    ...
    "이미 documents.matchReport가 있으면(매칭률 조회 배치가 먼저 작성한 것) 그 매칭률을 재사용해 70% 판정에 쓰고 matchReport는 다시 만들지 마. coreCompetency·coverLetter·intro·workExperience만 새로 채워 병합해.",
