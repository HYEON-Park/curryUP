...
// 저장 위치는 기존 documents.{docType} 한 곳을 그대로 재사용한다(새 필드·새 저장소 없음).
export const DOC_TYPES = ["matchReport", "coverLetter", "intro", "workExperience"] as const;
  ...
  switch (docType) {
    case "matchReport":
      return [
        ...
        ".claude/skills/write-documents/SKILL.md의 §6-1 매칭률 사전 평가 + §6-2 지원 권장도 형식으로 전체를 작성해:",
        "- 필수 자격요건/스택/담당업무/우대사항 매칭, 강점 Top 3, 갭 Top 3~5,",
        "- '종합 매칭률: N%' 한 줄 반드시 포함(대시보드가 이 문자열로 매칭률을 파싱한다),",
        "- 지원 권장도 및 다른 공고 대비 비교, §6-6 사용 시 체크포인트까지 모두 포함.",
        "저장 필드: documents.matchReport (문자열). coverLetter/intro/workExperience는 절대 건드리지 마.",
      ].join("\n");
    case "coverLetter":
      return [
        "생성 대상: documents.coverLetter (자기소개서).",
        "SKILL.md §6-4 형식으로 자기소개서 전체를 작성해: 지원동기(KKK 분석형) + 직무 역량(STAR-F, Action 굵은 불릿 3개) + 입사 후 포부(3단계 구조).",
        "§3 문체 규칙·§6-5 인간화 리라이팅·§7 자가 검증 체크리스트를 적용해.",
        "documents.matchReport가 이미 있으면 그 매칭 분석을 참고하되 matchReport는 다시 만들지 마.",
        "저장 필드: documents.coverLetter (문자열). matchReport/intro/workExperience는 절대 건드리지 마.",
      ].join("\n");
    case "intro":
      return [
        ...
        "§3 문체 규칙·금지어 규칙을 적용해.",
        "저장 필드: documents.intro (문자열). matchReport/coverLetter/workExperience는 절대 건드리지 마.",
      ].join("\n");
    case "workExperience":
      return [
        "생성 대상: documents.workExperience (공고 맞춤 경력기술서).",
        "SKILL.md §6-3 형식으로 경력기술서 전체를 작성해: 핵심 역량, 담당 업무, 경력 기술(프로젝트를 공고 관련성 순으로 재배치), 자격 사항, 개인 프로젝트/학습.",
        "저장 필드: documents.workExperience (문자열). matchReport/coverLetter/intro는 절대 건드리지 마.",
      ].join("\n");
  }
}
...
function runClaudeSingleDoc(userId: string, jobId: string, docType: DocType): Promise<void> {
  ...
  const prompt = [
    ...
    "저장 규칙:",
    '  - documents가 null이면 새로 만들되 { matchReport: "", coverLetter: "", intro: "", workExperience: "", generatedAt: <ISO시각> } 형태에서 요청 필드만 채워.',
